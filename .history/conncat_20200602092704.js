...
function selectTable(tableName, res, ifsucc) {
    ...
    return  pool.getConnection((err,conn)=>{
         conn.query("select * from " + tableName,(e,r)=>res.json(new Result({ data: r ,success:ifsucc})))
         conn.release()
    })
...
function addTbale() { 
    return  pool.getConnection((err,conn)=>{
        conn.query("INSERT INTO student(`name`,`sex`,`age`, `address`) VALUES ?" + tableName,(e,r)=>res.json(new Result({ data: r ,success:ifsucc})))
        conn.release()
   })
 }
