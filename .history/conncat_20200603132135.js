...
function selectTable(tableName, ctx) {
  // return (connQuery = pool.query("select * from " + tableName, (e, r) =>
  //   res.json(new Result({ data: r ,success:ifsucc}))
  // ));
  let a  = pool.getConnection((err, conn) => {
   return  conn.query("select * from " + tableName, (e, r) => {
      if (e) {
        succ = false;
        code = e.code;
      ...
      return 111;
      return new Result({ code, msg, data: r, success: succ });
    //    ctx.body = 123
